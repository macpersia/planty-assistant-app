...
abstract class AbstractAudioRecorder protected constructor(audioSource: Int, protected val sampleRate: Int) : AudioRecorder {
     ...
     */
    override lateinit var state: AudioRecorder.State
        protected set
     ...
     */
    override val isPausing: Boolean
        get() {
     ...
     */
    override// TODO: why 10?
    val rmsdb: Float
        get() {
            val sumOfSquares = getRms(length, mBuffer!!.size)
            val rootMeanSquare = Math.sqrt((sumOfSquares / (mBuffer!!.size / 2)).toDouble())
            return if (rootMeanSquare > 1) {
                (10 * Math.log10(rootMeanSquare)).toFloat()
            } else 0f
        }
    ...
    override fun start() {
        if (speechRecordState == AudioRecord.STATE_INITIALIZED) {
            ...
            if (mRecorder!!.recordingState == AudioRecord.RECORDSTATE_RECORDING) {
                state = AudioRecorder.State.RECORDING
                object : Thread() {
                    override fun run() {
    ...
    fun stop() {
        ...
        if (speechRecordState == AudioRecord.STATE_INITIALIZED && mRecorder!!.recordingState == AudioRecord.RECORDSTATE_RECORDING) {
            try {
                mRecorder!!.stop()
                state = AudioRecorder.State.STOPPED
            } catch (e: IllegalStateException) {
                handleError("native stop() called in illegal state: " + e.message)
            }
    ...
    protected open fun recorderLoop(recorder: SpeechRecord?) {
    ...
    protected fun handleError(msg: String) {
        release()
        state = AudioRecorder.State.ERROR
        Log.e(msg)
    }
