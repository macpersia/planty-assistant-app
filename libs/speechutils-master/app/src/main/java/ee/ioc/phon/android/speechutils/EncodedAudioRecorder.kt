 ...
 */
class EncodedAudioRecorder @JvmOverloads constructor(audioSource: Int = DEFAULT_AUDIO_SOURCE, sampleRate: Int = DEFAULT_SAMPLE_RATE) : AbstractAudioRecorder(audioSource, sampleRate) {
    ...
    init {
        try {
            ...
            val framePeriod = bufferSize / (2 * RESOLUTION_IN_BYTES * CHANNELS)
        ...
        mRecordingEnc = ByteArray(RESOLUTION_IN_BYTES * CHANNELS * sampleRate * 35) // 35 sec raw
    }

    constructor(sampleRate: Int) : this(DEFAULT_AUDIO_SOURCE, sampleRate) {}
    ...
    @Override
    protected fun recorderLoop(speechRecord: SpeechRecord) {
        mNumBytesSubmitted = 0
        mNumBytesDequeued = 0
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            val format = MediaFormatFactory.createMediaFormat(MediaFormatFactory.Type.FLAC, getSampleRate())
    ...
    private fun recorderEncoderLoop(codec: MediaCodec, speechRecord: SpeechRecord) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            ...
            var codecOutputBuffers = codec.getOutputBuffers()
            ...
            while (true) {
                ...
                if (index == MediaCodec.INFO_TRY_AGAIN_LATER) {
                    ...
                    val format = codec.getOutputFormat()
