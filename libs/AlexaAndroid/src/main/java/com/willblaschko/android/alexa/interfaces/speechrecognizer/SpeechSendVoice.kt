...
class SpeechSendVoice : SpeechSendEvent() {
    ...
    protected override val requestBody: RequestBody
    ...
    fun startRecording(url: String, accessToken: String, buffer: ByteArray?,
        ...
        synchronized(mLock) {
            mAudioRecord = AudioRecord(MediaRecorder.AudioSource.MIC, AUDIO_RATE, AudioFormat.CHANNEL_IN_MONO, AudioFormat.ENCODING_PCM_16BIT, BUFFER_SIZE)
        }
        ...
        val recordingThread = Thread(Runnable {
            ...
            while (mIsRecording) {
                ...
                if (count <= 0) {
                    ...
                    mIsRecording = false
                    if (mCallback != null) {
                        mCallback!!.failure(RuntimeException("audio read fail, error code:" + count))
                    }
                    break
                }
                try {
