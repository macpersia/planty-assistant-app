...
import be.planty.android.alexa.interfaces.GenericSendEvent
...
import be.planty.android.alexa.interfaces.response.ResponseParser.getBoundary
import be.planty.android.alexa.interfaces.speechrecognizer.SpeechSendAudio
import be.planty.android.alexa.interfaces.speechrecognizer.SpeechSendText
...
import be.planty.android.alexa.service.DownChannelService
...
class AlexaManager private constructor(context: Context, productId: String?) {
    ...
    lateinit var lwaRequestContext: RequestContext
        private set
    ...
    init {
        ...
        lwaRequestContext = RequestContext.create(mContext)
        ...
        authorizationManager = AuthorizationManager(mContext, productId, lwaRequestContext)
     ...
     */
    fun checkLoggedIn(callback: AsyncCallback<Boolean, AuthError?>) {
        authorizationManager.checkLoggedIn(mContext, object : AsyncCallback<Boolean, AuthError?> {
            override fun start() {}
            override fun success(result: Boolean) = callback.success(result)
            override fun failure(error: AuthError?) = callback.failure(error)
    ...
    /**
     * Send a synchronize state [Event] request to Alexa Servers to retrieve pending [be.planty.android.alexa.data.Directive]
     * See: [.sendEvent]
     * @param callback state callback
     */
...
//                            override fun onError(e: AuthError?) {
     ...
     * See: [.sendEvent]
     *
     * @param volume volume as reported by the [be.planty.android.alexa.interfaces.speaker.AvsAdjustVolumeItem] Directive
     ...
     * See: [.sendEvent]
     *
     * @param isMute mute state as reported by the [be.planty.android.alexa.interfaces.speaker.AvsSetMuteItem] Directive
     * @param callback
     */
    ...
    /**
     * Send a generic event to the AVS server, this is generated using [be.planty.android.alexa.data.Event.Builder]
...
//                            override fun onError(p0: AuthError?) {
    ...
    private class AsyncEventHandler(var manager: AlexaManager, var callback: AsyncCallback<AvsResponse, Exception?>?) : AsyncCallback<Call, Exception> {
        ...
        override fun success(currentCall: Call) {
            try {
                ...
                val items = if (response.code() == HttpURLConnection.HTTP_NO_CONTENT)
                    ...
                    ResponseParser.parseResponse(response.body()?.byteStream()!!,
                        getBoundary(response)
                    )
    ...
    private abstract class ImplAuthorizationCallback<E>(var callback: AsyncCallback<E, Exception?>?) : AuthorizationCallback {
    ...
    private abstract class ImplCheckLoggedInCallback : AsyncCallback<Boolean, AuthError?> {
